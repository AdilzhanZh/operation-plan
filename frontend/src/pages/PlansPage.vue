...
const activePanelSubtitle = computed(() => {
  if (activeViewTab.value === 'reports') {
    return tr(
      'Выберите индикатор и просмотрите все отправленные отчеты. Для сотрудника доступна повторная отправка.',
      'Индикаторды таңдап, жіберілген есептердің толық тарихын көріңіз. Қызметкер үшін қайта жіберу қолжетімді.'
    )
  }
  return tr(
    'Редактирование доступно администратору, отправка отчета - назначенному сотруднику.',
    'Өңдеу әкімшіге қолжетімді, есеп жіберу бекітілген қызметкерге арналған.'
  )
})
          ...
          <label class="modal-label">
            ...
            <div class="row-edit-prorectors">
              ...
              <p v-if="prorectors.length === 0" class="empty-state row-edit-prorectors-empty">
                {{ tr('Список сотрудников пуст.', 'Қызметкерлер тізімі жоқ.') }}
              </p>
            </div>
          </label>
        </div>
